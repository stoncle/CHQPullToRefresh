...
#define kPrefixTagNumber 15727

@interface MONActivityIndicatorView1 ()
...
@implementation MONActivityIndicatorView1
...
- (void)changeCirCle:(int)i withZoomRatio:(float)ratio
{
    ...
    {
        if(circle.tag - kPrefixTagNumber == i)
        {
            CGRect frame = circle.frame;
            CGPoint center = circle.center;
            CGFloat originalDiameter = 2*self.radius;
            [circle setFrame:CGRectMake(frame.origin.x, 0, originalDiameter*ratio, originalDiameter*ratio)];
            [circle setCenter:CGPointMake(center.x, center.y)];
            circle.layer.cornerRadius = circle.frame.size.width/2.0f;
        }
    }
}
